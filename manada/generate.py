...
def main():
	...
	tries = 0
	while nt < args.n:
		# Draw non-substructure parameters
		sample = sampler.sample()
		...
		# Get the model and kwargs from the source class and turn it
		# into a source model
		...
		for _ in range(config_module.repeat_non_substructure):
			# Draw substructure parameters
			sub_sample = sampler.sample()

			# We always try
			tries += 1

			meta_values = dict(
				source_parameters_catalog_i=catalog_i,
				source_parameters_phi=phi)
			...
			if 'los' in config_dict:
				los_class.update_parameters(
					sub_sample['los_parameters'],sub_sample['main_deflector_parameters'],
					sub_sample['source_parameters'],sub_sample['cosmology_parameters'])
			...
			if 'subhalo' in config_dict:
				subhalo_class.update_parameters(
					sub_sample['subhalo_parameters'],
					sub_sample['main_deflector_parameters'],
					sub_sample['source_parameters'],sub_sample['cosmology_parameters'])
