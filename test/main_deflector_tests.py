...
class PEMDTShearests(unittest.TestCase):
	...
	def test_check_parameterization(self):
		...
		# parameters raises a value error.
		with self.assertRaises(ValueError):
			PEMDShear(cosmology_parameters='planck18',
				main_deflector_parameters={})
...
class PEMDShearFourMultipoleTests(unittest.TestCase):
	...
	def test_check_parameterization(self):
		...
		with self.assertRaises(ValueError):
			PEMDShearFourMultipole(cosmology_parameters='planck18',
				main_deflector_parameters={'z_lens': 0.5,'gamma': 2.0,
					'theta_E': 1.0,'e1': 0.2,'e2': 0.1,'center_x': 0.2,
					'center_y': 0.1,'gamma1':0.0,'gamma2':0.0,'ra_0':0.0,
					'dec_0':0.0})
	...
	def test_draw_main_deflector(self):
		...
		main_deflector_parameters = {'z_lens': 0.5,'gamma': 2.0,
			'theta_E': 1.0,'e1': 0.2,'e2': 0.1,'center_x': 0.2,
			'center_y': 0.1,'gamma1':0.0,'gamma2':0.0,'ra_0':0.0,'dec_0':0.0,
			'mult2_a':0.01,'mult2_phi':0.2,'mult2_center_x':0.0,
			'mult2_center_y':0.01,'mult3_a':0.01,'mult3_phi':0.4,
			'mult3_center_x':0.01,'mult3_center_y':0.0,'mult4_a':0.02,
			'mult4_phi':-0.2,'mult4_center_x':0.0,'mult4_center_y':0.0}
		c = PEMDShearFourMultipole(cosmology_parameters='planck18',
		...
		self.assertListEqual(md_model_list,['EPL_NUMBA','SHEAR','MULTIPOLE',
			'MULTIPOLE','MULTIPOLE'])
